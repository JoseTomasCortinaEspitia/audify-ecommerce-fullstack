...
function ProductsGrid({ language, reloadKey = 0 }) {
  ...
  return <section className="products-grid">{products.map((product) => <ProductCard key={product.id} product={product} language={language} />)}</section>
}
...
function Home({ user, language, onLanguageChange, onLogout }) {
  const t = copy[language]
  return <main><StoreHeader user={user} language={language} onLanguageChange={onLanguageChange} onLogout={onLogout} /><header className="store-hero" id="audio"><div className="hero-copy"><p className="eyebrow light">{t.market}</p><h1>{t.hero}</h1><p>{t.heroText}</p><a className="hero-button" href="#productos">{t.explore}</a></div></header><section className="catalog-section" id="productos"><div className="section-heading"><div><p className="eyebrow">{t.collection}</p><h2>{t.featured}</h2></div><span>{t.featureText}</span></div><ProductsGrid language={language} /></section></main>
}

function AdminPanel({ session, language, onLanguageChange, onLogout }) {
  const t = copy[language], [form, setForm] = useState(emptyProductForm), [image, setImage] = useState(null), [categories, setCategories] = useState([]), [message, setMessage] = useState(''), [reloadKey, setReloadKey] = useState(0)
  ...
  const submit = async (event) => { event.preventDefault(); setMessage(t.creating); const data = new FormData(); Object.entries(form).forEach(([key, value]) => data.append(key, value)); if (image) data.append('image', image); try { const response = await axios.post(`${API_URL}/products`, data, { headers: { Authorization: `Bearer ${session.token}` } }); setMessage(response.data.message || t.created); setForm(emptyProductForm); setImage(null); event.target.reset(); setReloadKey((key) => key + 1) } catch (error) { setMessage(error.response?.data?.message || t.createError) } }
  return <main className="admin-layout"><StoreHeader user={session.user} language={language} onLanguageChange={onLanguageChange} onLogout={onLogout} /><section className="admin-grid"><article className="panel-card"><p className="eyebrow">{t.createProduct}</p><h2>{t.newProduct}</h2><form className="form" onSubmit={submit}><label>{t.productName}<input name="name" value={form.name} onChange={change} required /></label><div className="form-row"><label>{t.price}<input name="price" type="number" min="0" value={form.price} onChange={change} required /></label><label>{t.stock}<input name="stock" type="number" min="0" value={form.stock} onChange={change} required /></label></div><label>{t.category}<select name="categoryId" value={form.categoryId} onChange={change} required><option value="">{t.selectCategory}</option>{options}</select></label><label>{t.description}<textarea name="description" value={form.description} onChange={change} required /></label><label>{t.image}<input type="file" accept="image/*" onChange={(event) => setImage(event.target.files[0])} /></label><button className="primary-button">{t.createProduct}</button></form>{message && <p className="form-message">{message}</p>}</article><article className="panel-card preview-panel"><p className="eyebrow">{t.publicView}</p><h2>{t.published}</h2><ProductsGrid language={language} reloadKey={reloadKey} /></article></section></main>
}
