...
const copy = {
  es: {
    ...
    cart: 'Carrito', orders: 'Mis órdenes', viewDetails: 'Ver detalles', productDetails: 'Detalle del producto', addToCart: 'Agregar al carrito', adding: 'Agregando…', addedToCart: 'Producto agregado al carrito', quantity: 'Cantidad', outOfStock: 'Agotado', continueShopping: 'Seguir comprando', yourCart: 'Tu carrito', emptyCart: 'Tu carrito está vacío', unitPrice: 'Precio unitario', subtotal: 'Subtotal', total: 'Total', remove: 'Quitar', checkout: 'Confirmar compra', checkingOut: 'Creando orden…', orderCreated: 'Compra confirmada correctamente', orderHistory: 'Historial de órdenes', noOrders: 'Todavía no tienes órdenes', order: 'Orden', status: 'Estado', date: 'Fecha', items: 'productos', cartError: 'No se pudo actualizar el carrito', orderError: 'No se pudo crear la orden', backToProducts: 'Volver a productos',
  ...
  en: {
    ...
    cart: 'Cart', orders: 'My orders', viewDetails: 'View details', productDetails: 'Product details', addToCart: 'Add to cart', adding: 'Adding…', addedToCart: 'Product added to cart', quantity: 'Quantity', outOfStock: 'Out of stock', continueShopping: 'Continue shopping', yourCart: 'Your cart', emptyCart: 'Your cart is empty', unitPrice: 'Unit price', subtotal: 'Subtotal', total: 'Total', remove: 'Remove', checkout: 'Confirm purchase', checkingOut: 'Creating order…', orderCreated: 'Purchase confirmed successfully', orderHistory: 'Order history', noOrders: 'You do not have any orders yet', order: 'Order', status: 'Status', date: 'Date', items: 'items', cartError: 'The cart could not be updated', orderError: 'The order could not be created', backToProducts: 'Back to products',
...
function ProductCard({ product, language, actions, onOpen }) {
  const t = copy[language]
  return <article className={`product-card${onOpen ? ' product-card-clickable' : ''}`} onClick={onOpen ? () => onOpen(product) : undefined} onKeyDown={onOpen ? (event) => { if (event.key === 'Enter' || event.key === ' ') onOpen(product) } : undefined} role={onOpen ? 'link' : undefined} tabIndex={onOpen ? 0 : undefined}><div className="product-image">{product.imageUrl ? <img src={product.imageUrl} alt={product.name} /> : <span>{t.noImage}</span>}</div><div className="product-content"><p className="category">{product.category?.name}</p><h3>{product.name}</h3><p className="product-description">{product.description}</p><div className="product-footer"><strong>{formatPrice(product, language)}</strong><span>{product.stock} {t.available}</span></div>{onOpen && <span className="details-link">{t.viewDetails} →</span>}{actions && <div className="product-actions"><button type="button" className="edit-button" onClick={() => actions.onEdit(product)}>{t.edit}</button><button type="button" className="delete-button" disabled={actions.deletingId === product.id} onClick={() => actions.onDelete(product)}>{actions.deletingId === product.id ? '…' : t.delete}</button></div>}</div></article>
}
...
function Catalog({ language, onOpen }) {
  const t = copy[language]
  ...
  return <><div className="catalog-controls"><label className="search-control"><span>{t.search}</span><input type="search" value={search} placeholder={t.searchPlaceholder} onChange={(event) => setSearch(event.target.value)} /></label><label><span>{t.category}</span><select value={categoryId} onChange={(event) => setCategoryId(event.target.value)}><option value="">{t.allCategories}</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label><label><span>{t.sortBy}</span><select value={sort} onChange={(event) => setSort(event.target.value)}><option value="newest">{t.newest}</option><option value="price-low">{t.priceLow}</option><option value="price-high">{t.priceHigh}</option><option value="name">{t.nameAZ}</option></select></label></div><div className="catalog-summary"><span>{visibleProducts.length} {t.results}</span>{(search || categoryId || sort !== 'newest') && <button type="button" className="clear-button" onClick={clear}>{t.clearFilters}</button>}</div>{status ? <p className="status-message">{status === 'loading' ? t.loading : t.loadError}</p> : visibleProducts.length ? <section className="products-grid">{visibleProducts.map((product) => <ProductCard key={product.id} product={product} language={language} onOpen={onOpen} />)}</section> : <p className="status-message">{t.empty}</p>}</>
}

function StoreHeader({ user, language, onLanguageChange, onLogout, onNavigate, cartCount = 0 }) {
  const t = copy[language]
  const navigate = (event, path) => { if (!onNavigate) return; event.preventDefault(); onNavigate(path) }
  return <nav className="topbar"><a className="brand" href="/" onClick={(event) => navigate(event, '/')}>AUDIFY</a><div className="nav-links"><a href="/#productos" onClick={(event) => navigate(event, '/')}>{t.products}</a><a href="/orders" onClick={(event) => navigate(event, '/orders')}>{t.orders}</a></div><div className="account-menu"><a className="cart-link" href="/cart" onClick={(event) => navigate(event, '/cart')}>{t.cart}<span>{cartCount}</span></a>{user.avatarUrl && <img className="avatar" src={user.avatarUrl} alt="" />}<span>{user.name || user.email}</span><LanguageButton language={language} onChange={onLanguageChange} /><button className="link-button" onClick={onLogout}>{t.logout}</button></div></nav>
}

function Home({ user, language, onLanguageChange, onLogout, onNavigate, cartCount }) {
  const t = copy[language]
  return <main><StoreHeader user={user} language={language} onLanguageChange={onLanguageChange} onLogout={onLogout} onNavigate={onNavigate} cartCount={cartCount} /><header className="store-hero" id="audio"><div className="hero-copy"><p className="eyebrow light">{t.market}</p><h1>{t.hero}</h1><p>{t.heroText}</p><a className="hero-button" href="#productos">{t.explore}</a></div></header><section className="catalog-section" id="productos"><div className="section-heading"><div><p className="eyebrow">{t.collection}</p><h2>{t.featured}</h2></div><span>{t.featureText}</span></div><Catalog language={language} onOpen={(product) => onNavigate(`/products/${product.id}`)} /></section></main>
}

const authHeaders = (token) => ({ headers: { Authorization: `Bearer ${token}` } })

function ProductDetail({ productId, session, language, onLanguageChange, onLogout, onNavigate, cartCount, onCartChange }) {
  const t = copy[language], [product, setProduct] = useState(null), [status, setStatus] = useState('loading'), [quantity, setQuantity] = useState(1), [message, setMessage] = useState(''), [adding, setAdding] = useState(false)
  useEffect(() => { axios.get(`${API_URL}/products/${productId}`).then(({ data }) => { setProduct(data.product); setStatus('') }).catch(() => setStatus('error')) }, [productId])
  const add = async () => { setAdding(true); setMessage(''); try { await axios.post(`${API_URL}/cart/items`, { productId, quantity }, authHeaders(session.token)); setMessage(t.addedToCart); onCartChange() } catch (error) { setMessage(error.response?.data?.message || t.cartError) } finally { setAdding(false) } }
  return <main className="store-page"><StoreHeader user={session.user} language={language} onLanguageChange={onLanguageChange} onLogout={onLogout} onNavigate={onNavigate} cartCount={cartCount} /><section className="detail-shell">{status ? <p className="status-message">{status === 'loading' ? t.loading : t.loadError}</p> : <><button className="back-button" onClick={() => onNavigate('/')}>← {t.backToProducts}</button><article className="product-detail"><div className="detail-image">{product.imageUrl ? <img src={product.imageUrl} alt={product.name} /> : <span>{t.noImage}</span>}</div><div className="detail-content"><p className="eyebrow">{t.productDetails}</p><p className="category">{product.category?.name}</p><h1>{product.name}</h1><strong className="detail-price">{formatPrice(product, language)}</strong><p className="detail-description">{product.description}</p><p className="stock-line">{product.stock > 0 ? `${product.stock} ${t.available}` : t.outOfStock}</p><div className="buy-row"><label>{t.quantity}<input type="number" min="1" max={product.stock} value={quantity} onChange={(event) => setQuantity(Number(event.target.value))} /></label><button className="primary-button" disabled={adding || product.stock < 1 || quantity < 1 || quantity > product.stock} onClick={add}>{adding ? t.adding : t.addToCart}</button></div>{message && <p className={`form-message${message === t.addedToCart ? '' : ' error'}`}>{message}</p>}</div></article></>}</section></main>
}

function CartPage({ session, language, onLanguageChange, onLogout, onNavigate, cartCount, onCartChange }) {
  const t = copy[language], [items, setItems] = useState([]), [status, setStatus] = useState('loading'), [message, setMessage] = useState(''), [checkingOut, setCheckingOut] = useState(false)
  const load = useCallback(() => { axios.get(`${API_URL}/cart`, authHeaders(session.token)).then(({ data }) => { setItems(data.items || []); setStatus('') }).catch(() => setStatus('error')) }, [session.token])
  useEffect(load, [load])
  const update = async (productId, quantity) => { try { await axios.put(`${API_URL}/cart/items/${productId}`, { quantity }, authHeaders(session.token)); load(); onCartChange() } catch (error) { setMessage(error.response?.data?.message || t.cartError) } }
  const remove = async (productId) => { try { await axios.delete(`${API_URL}/cart/items/${productId}`, authHeaders(session.token)); load(); onCartChange() } catch (error) { setMessage(error.response?.data?.message || t.cartError) } }
  const checkout = async () => { setCheckingOut(true); setMessage(''); try { await axios.post(`${API_URL}/orders`, {}, authHeaders(session.token)); onCartChange(); onNavigate('/orders', { message: t.orderCreated }) } catch (error) { setMessage(error.response?.data?.message || t.orderError) } finally { setCheckingOut(false) } }
  const total = items.reduce((sum, item) => sum + item.product.price * item.quantity, 0)
  return <main className="store-page"><StoreHeader user={session.user} language={language} onLanguageChange={onLanguageChange} onLogout={onLogout} onNavigate={onNavigate} cartCount={cartCount} /><section className="commerce-shell"><div className="section-heading"><div><p className="eyebrow">{t.cart}</p><h1>{t.yourCart}</h1></div><button className="secondary-button" onClick={() => onNavigate('/')}>{t.continueShopping}</button></div>{status ? <p className="status-message">{status === 'loading' ? t.loading : t.loadError}</p> : !items.length ? <div className="empty-state"><h2>{t.emptyCart}</h2><button className="primary-button" onClick={() => onNavigate('/')}>{t.continueShopping}</button></div> : <><div className="cart-list">{items.map((item) => <article className="cart-item" key={item.id}><div className="cart-thumb">{item.product.imageUrl ? <img src={item.product.imageUrl} alt={item.product.name} /> : t.noImage}</div><div><p className="category">{item.product.category?.name}</p><h3>{item.product.name}</h3><span>{t.unitPrice}: {formatPrice(item.product, language)}</span></div><label>{t.quantity}<input type="number" min="1" max={item.product.stock} value={item.quantity} onChange={(event) => update(item.productId, Number(event.target.value))} /></label><strong>{formatPrice({ price: item.product.price * item.quantity }, language)}</strong><button className="remove-button" onClick={() => remove(item.productId)}>{t.remove}</button></article>)}</div><aside className="cart-summary"><span>{t.total}</span><strong>{formatPrice({ price: total }, language)}</strong><button className="primary-button" disabled={checkingOut} onClick={checkout}>{checkingOut ? t.checkingOut : t.checkout}</button></aside></>}{message && <p className="form-message error">{message}</p>}</section></main>
}

function OrdersPage({ session, language, onLanguageChange, onLogout, onNavigate, cartCount, navigationMessage }) {
  const t = copy[language], [orders, setOrders] = useState([]), [status, setStatus] = useState('loading')
  useEffect(() => { axios.get(`${API_URL}/orders`, authHeaders(session.token)).then(({ data }) => { setOrders(data.orders || []); setStatus('') }).catch(() => setStatus('error')) }, [session.token])
  return <main className="store-page"><StoreHeader user={session.user} language={language} onLanguageChange={onLanguageChange} onLogout={onLogout} onNavigate={onNavigate} cartCount={cartCount} /><section className="commerce-shell"><div className="section-heading"><div><p className="eyebrow">{t.orders}</p><h1>{t.orderHistory}</h1></div><button className="secondary-button" onClick={() => onNavigate('/')}>{t.continueShopping}</button></div>{navigationMessage && <p className="form-message">{navigationMessage}</p>}{status ? <p className="status-message">{status === 'loading' ? t.loading : t.loadError}</p> : !orders.length ? <div className="empty-state"><h2>{t.noOrders}</h2></div> : <div className="orders-list">{orders.map((order) => <article className="order-card" key={order.id}><header><div><p className="eyebrow">{t.order} #{order.id.slice(0, 8)}</p><strong>{new Intl.DateTimeFormat(language === 'es' ? 'es-CO' : 'en-US', { dateStyle: 'medium' }).format(new Date(order.createdAt))}</strong></div><span className="status-pill">{order.status}</span><strong>{formatPrice({ price: order.total }, language)}</strong></header><div className="order-items">{order.items.map((item) => <div key={item.id}><span>{item.quantity} × {item.product.name}</span><strong>{formatPrice({ price: item.price * item.quantity }, language)}</strong></div>)}</div></article>)}</div>}</section></main>
}
...
function App() {
  const [session, setSession] = useState(readSession), [language, setLanguage] = useState(() => localStorage.getItem('audify-language') || 'es'), [path, setPath] = useState(window.location.pathname), [cartCount, setCartCount] = useState(0), [navigationMessage, setNavigationMessage] = useState('')
  ...
  const navigate = useCallback((nextPath, state = {}) => { window.history.pushState({}, '', nextPath); setPath(nextPath); setNavigationMessage(state.message || ''); window.scrollTo({ top: 0, behavior: 'smooth' }) }, [])
  const refreshCart = useCallback(() => { if (!session) return; axios.get(`${API_URL}/cart`, authHeaders(session.token)).then(({ data }) => setCartCount((data.items || []).reduce((sum, item) => sum + item.quantity, 0))).catch(() => setCartCount(0)) }, [session])
  useEffect(() => { document.documentElement.lang = language }, [language])
  useEffect(() => { const pop = () => { setPath(window.location.pathname); setNavigationMessage('') }; window.addEventListener('popstate', pop); return () => window.removeEventListener('popstate', pop) }, [])
  ...
  if (path.startsWith('/products/')) return <ProductDetail productId={path.split('/')[2]} session={session} language={language} onLanguageChange={changeLanguage} onLogout={logout} onNavigate={navigate} cartCount={cartCount} onCartChange={refreshCart} />
  if (path === '/cart') return <CartPage session={session} language={language} onLanguageChange={changeLanguage} onLogout={logout} onNavigate={navigate} cartCount={cartCount} onCartChange={refreshCart} />
  if (path === '/orders') return <OrdersPage session={session} language={language} onLanguageChange={changeLanguage} onLogout={logout} onNavigate={navigate} cartCount={cartCount} navigationMessage={navigationMessage} />
  return <Home user={session.user} language={language} onLanguageChange={changeLanguage} onLogout={logout} onNavigate={navigate} cartCount={cartCount} />
}
